...
struct EVY {
    ...
    static func submit(key: String) throws -> Void {
        if let existing = EVY.data.get(key: key) {
    ...
    static func updateValue(_ value: String, at: String) throws -> Void {
        ...
        guard let modelData = EVY.data.get(key: variables.first!) else {
            throw EVYDataError.keyNotFound
        }
        ...
        try EVY.data.update(key: variables.first!, data: newData)
    }
    ...
    static func parseProps(_ input: String) throws -> EVYJson? {
        ...
        if variables.count > 0 {
            ...
            if let data = EVY.data.get(key: firstVariable) {
