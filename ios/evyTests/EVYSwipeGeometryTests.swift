...
final class EVYSwipeGeometryTests: XCTestCase {
  ...
  func testRevealWidthScalesWithActionCount() {
    XCTAssertEqual(
      EVYSwipeGeometry.revealWidth(actionCount: 1), EVYSwipeGeometry.revealWidth)
    XCTAssertEqual(
      EVYSwipeGeometry.revealWidth(actionCount: 2), EVYSwipeGeometry.revealWidth * 2)
  }

  /// A row with no actions is never made swipeable, but the width must stay positive
  /// rather than collapsing the reveal to nothing if one ever is.
  func testRevealWidthNeverCollapses() {
    XCTAssertEqual(
      EVYSwipeGeometry.revealWidth(actionCount: 0), EVYSwipeGeometry.revealWidth)
  }

  func testDragOffsetFromOpenStartsAtTwoButtonRevealWidth() {
    let twoButtons = EVYSwipeGeometry.revealWidth(actionCount: 2)
    let offset = EVYSwipeGeometry.dragOffset(
      translation: CGSize(width: 0, height: 0),
      isOpen: true,
      revealWidth: twoButtons
    )
    XCTAssertEqual(offset, -twoButtons)
  }

  /// Rubber-banding is relative to the row's own reveal, so a two-button row can be
  /// dragged twice as far before it starts resisting.
  func testDragOffsetRubberBandsBeyondTwoButtonStretch() {
    let twoButtons = EVYSwipeGeometry.revealWidth(actionCount: 2)
    let offset = EVYSwipeGeometry.dragOffset(
      translation: CGSize(width: -(twoButtons * 2) - 100, height: 0),
      isOpen: false,
      revealWidth: twoButtons
    )
    XCTAssertGreaterThan(offset, -(twoButtons * 2) - 100)
    XCTAssertLessThan(offset, -(twoButtons * 2))
  }

  func testRevealButtonWidthStretchesForTwoButtons() {
    let twoButtons = EVYSwipeGeometry.revealWidth(actionCount: 2)
    XCTAssertEqual(
      EVYSwipeGeometry.revealButtonWidth(for: -10, revealWidth: twoButtons), twoButtons)
    XCTAssertEqual(
      EVYSwipeGeometry.revealButtonWidth(for: -(twoButtons + 30), revealWidth: twoButtons),
      twoButtons + 30)
  }
}
