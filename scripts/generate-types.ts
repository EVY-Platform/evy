...
async function generateSwift(schemaFiles: LoadedSchemaFile[]): Promise<void> {
	...
	const schemaFilesToQuicktype = schemaFiles.filter(
		(f) =>
			...
			f.schemaKey !== "rpc/cancel-image-upload.response" && // const:true unsupported by quicktype
			f.schemaKey !== "rpc/delete-image.response", // const:true unsupported by quicktype
	);
