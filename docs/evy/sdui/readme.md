...
- Resource/local data is loaded outside the flow document. Clients can request individual lists with JSON-RPC `get` (`service` / `resource`) using optional `filter.id` or `filter.updatedAfter`, or sync all changed data in batches with `sync`.
