...
- Collection/list sources and direct entity/draft attributes use canonical resource IDs (for example, `{[resource_id]}` and `{[resource_id].title}`). iOS and runtime clients resolve those IDs directly from synced data. Legacy plural and singular resource names may still resolve for local data backwards compatibility, but new SDUI should use resource IDs.
