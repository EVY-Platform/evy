...
Top-level `status` is `pending` or `accepted`. New messages are created `pending`; existing rows were backfilled to `pending` via marketplace migration `0003_message_status`. On the iOS homepage, accepting a message is authored as a `slide-left` `{update(...)}` that sets `status` to `accepted` when the current value is `pending`.
