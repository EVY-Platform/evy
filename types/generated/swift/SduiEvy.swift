...
//
//   let sduiEvy = try SduiEvy(json)

import Foundation

/// SDUI types: SDUI_Flow, SDUI_Page, SDUI_Row
// MARK: - SduiEvy
struct SduiEvy: Codable {
    let data, id: String
    let name: String
    let pages: [EvySchema]
    let type: SduiEvyType
}

// MARK: SduiEvy convenience initializers and mutators

extension SduiEvy {
    init(data: Data) throws {
        self = try newJSONDecoder().decode(SduiEvy.self, from: data)
    }
...
enum SduiEvyType: String, Codable {
    case create = "create"
    case delete = "delete"
    case read = "read"
    case update = "update"
