...
//
//   let rPCGetResponse = try RPCGetResponse(json)

import Foundation

/// Response for get method (SDUI: array of SDUI_Flow; other: key-value object)
enum RPCGetResponse: Codable {
    case sduiFlowArray([SDUIFlow])
    case unionMap([String: GetDataResponseValue])

    init(from decoder: Decoder) throws {
        let container = try decoder.singleValueContainer()
        if let x = try? container.decode([SDUIFlow].self) {
            self = .sduiFlowArray(x)
            return
        }
        if let x = try? container.decode([String: GetDataResponseValue].self) {
            self = .unionMap(x)
            return
        }
        throw DecodingError.typeMismatch(RPCGetResponse.self, DecodingError.Context(codingPath: decoder.codingPath, debugDescription: "Wrong type for RPCGetResponse"))
    }

    func encode(to encoder: Encoder) throws {
        var container = encoder.singleValueContainer()
        switch self {
        case .sduiFlowArray(let x):
...
enum GetSDUIResponseType: String, Codable {
    case create = "create"
    case delete = "delete"
    case read = "read"
    case update = "update"
...
enum GetDataResponseValue: Codable {
    case bool(Bool)
    case double(Double)
    case integer(Int)
    case string(String)
    case unionArray([GetDataResponseElement])
    case unionMap([String: GetDataResponseElement])
    case null
    ...
    init(from decoder: Decoder) throws {
        ...
        if let x = try? container.decode([GetDataResponseElement].self) {
            self = .unionArray(x)
            return
        }
        ...
        if let x = try? container.decode([String: GetDataResponseElement].self) {
            self = .unionMap(x)
            return
        }
        ...
        throw DecodingError.typeMismatch(GetDataResponseValue.self, DecodingError.Context(codingPath: decoder.codingPath, debugDescription: "Wrong type for GetDataResponseValue"))
    }
...
enum GetDataResponseElement: Codable {
    case bool(Bool)
    case double(Double)
    case integer(Int)
    case string(String)
    ...
    init(from decoder: Decoder) throws {
        ...
        throw DecodingError.typeMismatch(GetDataResponseElement.self, DecodingError.Context(codingPath: decoder.codingPath, debugDescription: "Wrong type for GetDataResponseElement"))
    }
