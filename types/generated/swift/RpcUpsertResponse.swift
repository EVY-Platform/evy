...
//
//   let rPCUpsertResponse = try RPCUpsertResponse(json)

import Foundation

/// Response for upsert (DATA_Flow | DATA_Data)
// MARK: - RPCUpsertResponse
struct RPCUpsertResponse: Codable {
    let createdAt: Date
    let data: SDUIFlow
    let id: String
    let updatedAt: Date
...
// MARK: RPCUpsertResponse convenience initializers and mutators

extension RPCUpsertResponse {
    init(data: Data) throws {
        self = try newJSONDecoder().decode(RPCUpsertResponse.self, from: data)
    }
